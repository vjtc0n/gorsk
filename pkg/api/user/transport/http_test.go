...
import (
	...
	"github.com/veep-provider/pkg/api/user/transport"
...
func TestCreate(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			rbac: &mock.RBAC{
				AccountCreateFn: func(c echo.Context, roleID veep.AccessRole, companyID, locationID int) error {
...

		{
			...
			udb: &mockdb.User{
				CreateFn: func(db orm.DB, usr veep.User) (*veep.User, error) {
					usr.ID = 1
...
func TestList(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			rbac: &mock.RBAC{
				UserFn: func(c echo.Context) *veep.AuthUser {
		...
		},
		{
			...
			udb: &mockdb.User{
				ListFn: func(db orm.DB, q *veep.ListQuery, p *veep.Pagination) ([]veep.User, error) {
		...
		},
		{
			...
			udb: &mockdb.User{
				ViewFn: func(db orm.DB, id int) (*veep.User, error) {
					return &veep.User{
						Role: &veep.Role{
							AccessLevel: veep.CompanyAdminRole,
						},
					}, nil
				},
			},
